...
use dap_plugin_api::{AdapterSpawn, PluginManifest, SpawnTransport, load_from_file};
...
async fn headless_session_init_over_control_client() {
    ...
    let result = run_session_init(
        &mut client,
        &SessionInitConfig::new("main.py")
            .with_adapter_id("fake")
            .with_manifest(fake_manifest()),
    )
    .await
    .expect("session init");
