...
pub struct MultiplexOptions {
    ...
    /// Active plugin metadata for `dapProxyPluginInfo` and initialize capability injection.
    pub plugin_context: Option<ProxyPluginContext>,
    /// Default attach/launch fields from an RSP `target.yaml` manifest.
    pub attach_defaults: Option<Value>,
}
...
pub async fn start_multiplexed_proxy(
    ...
    let plugin_context = options.plugin_context.clone();
    let attach_defaults = options.attach_defaults.clone();
...
pub async fn start_headless_multiplexed_proxy(
    ...
    let control_port = match options.control_port {
        Some(port) => Some(serve_control_attach(session.clone(), port, plugin_context).await?),
        None => None,
    };
    ...
    // this handle (see `HeadlessSession::drop`). Shutting down immediately would
    // tear down the adapter before the control client can send `initialize`.
    let join = tokio::spawn(async move {
        let _keepalive = (backend_bridge, mux_task);
...
async fn run_client_bridge(
    ...
    plugin_context: Option<ProxyPluginContext>,
) -> Result<()> {
    ...
    let write_for_ingest = write.clone();
    let ingest = tokio::spawn(async move {
        loop {
            ...
            if let Some(context) = plugin_context.as_ref() {
                if let Some(response) = try_handle_client_request(&message, context) {
                    write_for_ingest.lock().await.send(&response).await?;
                    continue;
                }
            }
            session
...
fn spawn_backend_bridge(
    ...
    plugin_context: Option<ProxyPluginContext>,
    attach_defaults: Option<Value>,
) -> BackendBridge {
    let (shutdown_tx, shutdown_rx) = tokio::sync::oneshot::channel();
    let join = tokio::spawn(async move {
        if let Err(err) = bridge_backend(
            backend,
            to_mux,
            from_mux,
            child_sessions,
...
async fn bridge_backend(
    ...
    let backend_to_mux = tokio::spawn(async move {
        while let Some(message) = backend_read.recv().await? {
            ...
            if plugin_context.is_some() {
                augment_initialize_response(&mut outbound);
            }
            to_mux
                .send(message_to_value(&outbound)?)
        ...
        loop {
            tokio::select! {
                value = from_mux.recv() => {
                    match value {
                        Some(value) => {
                            let mut message = value_to_message(value)?;
                            if let Some(defaults) = attach_defaults.as_ref() {
                                merge_debug_request_arguments(&mut message, defaults);
                            }
