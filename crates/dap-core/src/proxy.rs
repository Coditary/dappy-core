...
impl Backend {
    ...
    pub async fn spawn_with_options(
        ...
        match adapter.transport {
            ...
            SpawnTransport::Builtin => crate::builtin_adapter::spawn_builtin(adapter).await,
        }
    }

    pub fn from_duplex(duplex: DuplexChannel) -> Self {
        Self {
            duplex,
...
/// Default builtin plugin directory (`dap-plugins/builtin`).
pub fn default_plugins_dir() -> PathBuf {
    dap_plugin_api::default_builtin_dir()
}

/// Load plugin manifests from a plugin root directory.
