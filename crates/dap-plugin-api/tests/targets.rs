use dap_plugin_api::{default_builtin_dir, load_target_defaults, load_target_from_file, target_manifest_in_dir};
...
fn load_target_defaults_includes_builtin_entries() {
    ...
    assert!(manifests.iter().any(|manifest| manifest.id == "qemu-x86-kernel"));
