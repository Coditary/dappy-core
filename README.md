...
Core backend for [Dappy](https://github.com/Coditary/Dappy): DAP proxy libraries and the `dap-proxy` editor binary.
...
| [`crates/dap-plugin-api`](crates/dap-plugin-api/) | Manifest loader |
...
Adapter manifests ship with the [Dappy](https://github.com/Coditary/Dappy) CLI project (`dap-plugins/builtin`). Set `DAP_PLUGINS_DIR` when running `dap-proxy` standalone.
