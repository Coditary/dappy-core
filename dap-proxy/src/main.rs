...
use dap_core::{
    ...
    ParentSessionContext, SessionInitConfig, arm_parent_death_signal, run_debug_request,
...
struct Cli {
    ...
    /// Builtin adapter plugin id (from plugins/builtin/*.yaml). Ignored when `--program` triggers routing.
    #[arg(long)]
    ...
    /// Directory with `*.yaml` plugin manifests.
...
async fn run_editor_proxy(cli: Cli) -> Result<()> {
    ...
    let mux_options = build_mux_options(&cli);
...
async fn run_headless(cli: Cli) -> Result<()> {
    ...
    let debug_args = serde_json::from_str::<Value>(&cli.debug_args)
    ...
    let init_config = SessionInitConfig::new(cli.program.clone().unwrap_or_else(|| "child".into()))
