...
┌─────────────────────────────────────────────────────────────────┐
│ Binaries                                                         │
│  dap-proxy (editor stdio)    dap-cli (control + MCP)             │
└────────────┬──────────────────────────┬──────────────────────────┘
             │                          │
┌────────────▼──────────────────────────▼──────────────────────────┐
...
│ DAP JSON + framing      │    │ YAML loader (no adapters) │
└─────────────────────────┘    └───────────────────────────┘
             │
┌────────────▼────────────────────────────────────────────────────┐
...
`dap-cli debug mcp` — built-in MCP server (stdio); talks directly to `ReplContext`

reference/dapper — vendored Meta Dapper (MIT), not built; hidden from IDE via `.gitignore`

Sibling [`dap-plugins`](../../dap-plugins/) — YAML manifests only (`builtin/<id>/plugin.yaml`, or `$DAP_PLUGINS_DIR`)
```
...
| 4 | `session_init`, `ExecutionStateTracker`, built-in MCP server (`dap-cli debug mcp`) |
...
- MCP tools → `dap-cli debug mcp` (Phase 5)
