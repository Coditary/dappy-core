    ...
    match source {
        Some(text) => {
            out.push_str(&format_source_snippet_styled(
                ...
                &style,
                language_for_path(&location.path),
            ));
        }
        None => {
...
fn format_source_snippet_styled(
    ...
    let snippet_lines = &lines[start - 1..end];
    let snippet_text = snippet_lines.join("\n");
    let highlighted_snippet = language
        .and_then(|lang| crate::tree_sitter::highlight_snippet(lang, &snippet_text, style));

    let mut out = String::new();
    for (offset, line_no) in (start..=end).enumerate() {
        ...
        let text = highlighted_snippet
            .as_ref()
            .and_then(|highlighted| highlighted_line(highlighted, offset, snippet_lines.len()))
            .unwrap_or_else(|| highlight_python_line(lines[line_no - 1], style));
        out.push_str(&format!("  {marker} {line_num} | {text}\n"));
    }
    out
}

fn highlighted_line(highlighted: &str, line_index: usize, line_count: usize) -> Option<String> {
    let parts = highlighted.split('\n').collect::<Vec<_>>();
    if parts.len() != line_count {
        return None;
    }
    parts.get(line_index).map(|line| line.to_string())
}
