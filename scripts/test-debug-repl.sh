...
ensure_built() {
  ...
  (cd "$ROOT" && env -u CARGO_TARGET_DIR cargo build -q -p dap-cli -p fake-dap-adapter)
}

ensure_built_cli() {
  log "Building dap-cli..."
  (cd "$ROOT" && env -u CARGO_TARGET_DIR cargo build -q -p dap-cli)
}
...
resolve_bin() {
  local name="$1"
  echo "$(target_dir)/debug/${name}"
}

run_dap_cli() {
  PATH="$(target_dir)/debug:${PATH}" RUST_LOG=off "$(target_dir)/debug/dap-cli" "$@"
}
